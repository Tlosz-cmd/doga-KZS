...
function checkQuotes() {
    if (typeof idezetek !== 'undefined' && idezetek.length > 0) {
        displayQuote(currentIndex);  
    } else {
        console.error("A idezetek.js nem töltődött be megfelelően, vagy nincsenek idézetek.");
    }
}


function displayQuote(index) {
    const idezet = idezet[index];
    document.getElementById('idezet-text').innerText = `"${idezetek.text}"`;
    document.getElementById('idezet-author').innerText = `- ${idezetek.author}`;
...
y
document.getElementById('favorite-btn').addEventListener('click', addFavorite);


document.addEventListener('DOMContentLoaded', checkQuotes);
